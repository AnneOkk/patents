...
library("readxl")
getwd()
data <- read_excel("../../data/Search_results/2022-11-07_exported.xlsx", sheet = "Sheet1")


duplicated(data$`Publication Number`)
```
...
citing_pat <- data_cite$`Publication Number`

data_cite <- data_cite %>% 
  select(`Publication Number`, `Citing Patents` )  %>% 
  filter(.$`Citing Patents` %in% citing_pat) %>%
  fastDummies::dummy_cols(., select_columns = "Citing Patents") %>% 
  select(-`Citing Patents`) %>% data.frame(.)

row.names(data_cite) <- data_cite$Publication.Number
...
ap_lda <- LDA(TextDoc_dtm, k = 20, control = list(seed = 1234))
...
diag(data_cpc_co_crossprod) <- 0       # (b/c you don't count co-occurrences of an aspect with itself)
...
data_links <- data_cpc_co_crossprod %>%  
  ...
  rename(source = Pub_Num, 
         target = name,
         importance = value) %>%                                     # Consolidate duplicates
...
# Make the plot
plot(network, vertex.color=my_color, edge.width=E(network)$importance*0.5 )

data$`CPC - Current - DWPI`

```


```{r}
data_cpc_co_crossprod <- column_to_rownames(data_cpc_co_crossprod, var = "Pub_Num")
data_cpc_co_crossprod[is.na(data_cpc_co_crossprod)] <- 0

bip = network(data_cpc_co_crossprod,
...
# create CPC df
data_cite <- separate_rows(data,`Citing Patents`,sep = " | ")
data_cite <- data_cite[!grepl("\\|", data_cite$`Citing Patents`),] 
data_cite <- data_cite[!is.na(data_cite$`Citing Patents`),]
```
...
cat("Patent 1:", strat_imp$`Title - DWPI`[1])

cat("Patent 2:", strat_imp$`Title - DWPI`[2])

cat("Patent 3:", strat_imp$`Title - DWPI`[3])

cat("Patent 4:", strat_imp$`Title - DWPI`[4])

cat("Patent 5:", strat_imp$`Title - DWPI`[5])
```
...
cat("Patent 1:", grant_prob$`Title - DWPI`[1])

cat("Patent 2:", grant_prob$`Title - DWPI`[2])

cat("Patent 3:", grant_prob$`Title - DWPI`[3])

cat("Patent 4:", grant_prob$`Title - DWPI`[4])

cat("Patent 5:", grant_prob$`Title - DWPI`[5])
```
...
cat("Patent 1:", dom_inf$`Title - DWPI`[1])

cat("Patent 2:", dom_inf$`Title - DWPI`[2])

cat("Patent 3:", dom_inf$`Title - DWPI`[3])

cat("Patent 4:", dom_inf$`Title - DWPI`[4])

cat("Patent 5:", dom_inf$`Title - DWPI`[5])
```

...
library("readxl")
getwd()
data <- read_excel("../../data/Search_results/2022-11-09_exported.xlsx", sheet = "Sheet1")
```
...
data$Data <- gsub("Mental Health", "Clinical History", data$Data)
data$Data <- gsub("Physical Health", "Clinical History", data$Data)
data$Data <- gsub("Clinical Records", "Clinical History", data$Data)
```


```{r}
disord.split <- strsplit(data$`Illness group`, ",") 
lev <- unlist(disord.split) %>%  trimws(.) %>%  unique(.)
disord.dummy <- lapply(disord.split, function(x) table(factor(x, levels=lev)))

pp.split <- strsplit(data$`Precision psychiatry domain`, ",") 
lev <- unlist(pp.split) %>%  trimws(.) %>%  unique(.)
pp.dummy <- lapply(pp.split, function(x) table(factor(x, levels=lev)))

feed.split <- strsplit(data$`Feedback type (in case of treatment/ predictive)`, ",") 
lev <- unlist(feed.split) %>%  trimws(.) %>%  unique(.)
feed.dummy <- lapply(feed.split, function(x) table(factor(x, levels=lev)))

interv.split <- strsplit(data$`Intervention type`, ",") 
lev <- unlist(interv.split) %>%  trimws(.) %>%  unique(.)
interv.dummy <- lapply(interv.split, function(x) table(factor(x, levels=lev)))

data.split <- strsplit(data$Data, ",") 
lev <- unlist(data.split) %>%  trimws(.) %>%  unique(.)
data.dummy <- lapply(data.split, function(x) table(factor(x, levels=lev)))
...
data_dummy <-  cbind(data, do.call(rbind, disord.dummy), do.call(rbind, pp.dummy), do.call(rbind, feed.dummy), do.call(rbind, interv.dummy), do.call(rbind, data.dummy))
```
...
```{r}
data_dummy_data_illness <- data_dummy_num %>% select()
```
...
data_links <- data_cpc_co_crossprod %>%  
  ...
  as.data.frame() %>% 
  .[!duplicated(data.frame(t(apply(.[1:2], 1, sort)))),] %>% 
  filter(importance > 5)  #set value for minimum coo-ccureences
